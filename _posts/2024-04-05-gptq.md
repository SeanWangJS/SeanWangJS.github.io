...
$$
  \begin{aligned}
  &\argmin_q \frac{1}{2} \delta W^\top H \delta W \\ 
...
$$
q = \argmin_{q} \frac{w_q^2}{[H^{-1}]_{qq}} \qquad \qquad (4)
$$
...
$$
\argmin_{\hat{W}} \mid\mid WX - \hat{W} X\mid\mid^2
$$
...
设 \\(\delta L_i = \mid\mid W_{i,:} X - \hat{W}_{i,:} X\mid\mid^2\\)，则 \\(\delta L = \sum_{i=1}^{d_{row}} \delta L_i\\)，并且有
...
$$
q = \argmin_{q} \frac{(quant(w_q) - w_q)^2}{[H^{-1}]_{qq}} \qquad \qquad (7)
$$
