...
在半群的基础上，如果其中的二元运算存在单位元，即，\\(\exist e \in G\\)，\\(\forall x \in G\\)，使得 \\(x\star e = e \star x= x\\)，则称这样的结构为**幺半群**。
...
在半群的基础上，如果还满足逆元素性质，即 \\(\forall x \in G, \quad\exist x^{-1} \in G\\) ，使得 \\(x\star x^{-1} = e\\)，则称这样的结构为**群**。
...
* \\(\forall f: x \rightarrow y , \exist g \in hom(\mathbb{D})\\)，使得 \\(F(f) =g: F(x) \rightarrow F(y) \\)
